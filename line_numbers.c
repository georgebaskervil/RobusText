                       ...
                       int window_height)
{
    ...
    line_nums->rect =
        (SDL_Rect){0, 0, line_nums->width, window_height - 24}; // Subtract status bar height
                         ...
                         int first_visible_line, int visible_lines)
{
    ...
    if (total_lines != line_nums->line_count || line_nums->needs_update) {
        ...
        SDL_Surface *surface = SDL_CreateRGBSurface(0, line_nums->width, surface_height, 32, 0, 0,
                                                    0, 0); // Let SDL determine the masks
        ...
        if (surface) {
            ...
            for (int i = 0; i < visible_lines && (first_visible_line + i) <= total_lines; i++) {
                ...
                SDL_Surface *text_surface =
                    TTF_RenderText_Blended(line_nums->font, line_str, text_color);
                if (text_surface) {
                    // Calculate y position to align with text area
                    // Text area starts at 20px (margin), so first line number should align with
                    // first text line
                    int y_pos =
                        i * line_height + TEXT_MARGIN; // Align with text area starting position
                    if (y_pos < TEXT_MARGIN)
                        y_pos = TEXT_MARGIN; // Ensure minimum position

                    SDL_Rect dst_rect = {line_nums->width - text_surface->w -
                                             LINE_NUMBERS_PADDING, // Right-aligned
...
{
    if (!line_nums->enabled || !line_nums->texture)
        return;

    // Ensure rect has valid coordinates
    if (line_nums->rect.x < 0)
        line_nums->rect.x = 0;
    if (line_nums->rect.y < 0)
        line_nums->rect.y = 0;
    if (line_nums->rect.w <= 0 || line_nums->rect.h <= 0)
        return;
    ...
    SDL_FRect line_rect_f = {(float) line_nums->rect.x, (float) line_nums->rect.y,
                             (float) line_nums->rect.w, (float) line_nums->rect.h};
