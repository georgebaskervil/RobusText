...
static SDL_Rect draw_dialog_background(int dialog_width, int dialog_height, const char *title) {
    if (!dialog_ctx.renderer || !dialog_ctx.font) return (SDL_Rect){0, 0, 0, 0};
    ...
    if (title && strlen(title) > 0) {
        ...
        if (title_surface) {
            SDL_Texture *title_texture = SDL_CreateTextureFromSurface(dialog_ctx.renderer, title_surface);
            if (title_texture) {
                SDL_Rect title_rect = {
                    x + 10,
                    y + 10,
...
static SDL_Rect draw_button(int x, int y, int width, int height, const char *text, bool highlighted) {
    ...
    SDL_Color text_color = highlighted ? (SDL_Color){255, 255, 255, 255} : (SDL_Color){20, 20, 20, 255};
...
DialogResult show_save_confirmation_dialog(const char *filename) {
    ...
    while (dialog_running) {
            ...
            if (event.type == SDL_QUIT) {
                ...
                if (event.key.keysym.sym == SDLK_ESCAPE) {
                    ...
                    result = (selected_button == 0) ? DIALOG_YES : 
                            (selected_button == 1) ? DIALOG_NO : DIALOG_CANCEL;
                    dialog_running = false;
                }
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    ...
                    if (event.button.y >= button_y && event.button.y <= button_y + button_height) {
                        ...
                        else if (event.button.x >= no_x && event.button.x <= no_x + button_width) {
                            result = DIALOG_NO;
                            dialog_running = false;
                        }
                        else if (event.button.x >= cancel_x && event.button.x <= cancel_x + button_width) {
                            result = DIALOG_CANCEL;
                            dialog_running = false;
                        }
                    }
        ...
        // Draw dialog
        SDL_Rect dialog_rect = draw_dialog_background(dialog_width, dialog_height, "Unsaved Changes");
        ...
        draw_text(dialog_rect.x + 10, dialog_rect.y + 65, "Do you want to save before continuing?", text_color);
        
        // Draw buttons
        int button_y = dialog_rect.y + dialog_height - 50;
        draw_button(dialog_rect.x + 20, button_y, button_width, button_height, "Yes", selected_button == 0);
        draw_button(dialog_rect.x + 120, button_y, button_width, button_height, "No", selected_button == 1);
        draw_button(dialog_rect.x + 220, button_y, button_width, button_height, "Cancel", selected_button == 2);
...
char* show_save_as_dialog(void) {
    ...
    while (dialog_running) {
        ...
        if (cursor_blink < 30) {
            ...
            SDL_RenderDrawLine(dialog_ctx.renderer, 
                             input_rect.x + 5 + text_width, input_rect.y + 3,
                             input_rect.x + 5 + text_width, input_rect.y + input_height - 3);
        }
        
        // Draw buttons
        int button_y = dialog_rect.y + dialog_height - 50;
        draw_button(dialog_rect.x + dialog_width - 190, button_y, button_width, button_height, "Save", false);
        draw_button(dialog_rect.x + dialog_width - 90, button_y, button_width, button_height, "Cancel", false);
...
char* show_open_dialog(void) {
    ...
    while (dialog_running) {
        ...
        // Draw buttons
        int button_y = dialog_rect.y + dialog_height - 50;
        draw_button(dialog_rect.x + dialog_width - 190, button_y, button_width, button_height, "Open", false);
        draw_button(dialog_rect.x + dialog_width - 90, button_y, button_width, button_height, "Cancel", false);
