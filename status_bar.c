...
void init_status_bar(StatusBar *status, SDL_Renderer *renderer, TTF_Font *font, int window_width, int window_height) {
    (void)renderer;  // Currently unused but kept for future extensibility
    status->texture = NULL;
    status->font = font;
    status->height = STATUS_BAR_HEIGHT;
    status->rect = (SDL_Rect){0, window_height - STATUS_BAR_HEIGHT, window_width, STATUS_BAR_HEIGHT};
...
void update_status_bar(StatusBar *status, SDL_Renderer *renderer, 
                       const DocumentState *doc, const SearchState *search,
                       int cursor_pos, const char *text, int window_width) {
    
    // Update rect width for window resizing
    status->rect.w = window_width;
    
    if (!status->needs_update) return;
    ...
    SDL_Surface *bg_surface = SDL_CreateRGBSurface(0, window_width, STATUS_BAR_HEIGHT, 32,
    ...
    // Fill background
    SDL_FillRect(bg_surface, NULL, SDL_MapRGB(bg_surface->format, bg_color.r, bg_color.g, bg_color.b));
